 ...
 public class Launcher_5159_v1 {
     ...
     // Servos
     public Servo srvoDropper = null;
     public Servo srvoLeftPitstop = null;
     public Servo srvoRightPitstop = null;

     // Motor constants
     double dPwrIntake = 0.85;
     double dPwrCarousel = -0.1;
     double dPwrShooterUp = 0.2;
     double dPwrShooterDown = -0.1;
     double dPwrElevUp = 0.2;
     double dPwrElevDown = -0.1;

     // Servo constants
     double dPitstopUpPos = 0.57;
     double dPitstopDownPos = 0.45;
     double dDropperUpPos = 0.45;
     double dDropperDownPos = 0.0;
     ...
     public void initLauncher(OpMode opMode, String strElevName,
         ...
         mtrShooter.setDirection(DcMotor.Direction.REVERSE); // Positive input rotates counter clockwise
         mtrShooter.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
         mtrShooter.setMode(DcMotor.RunMode.RUN_USING_ENCODER);

         // Dropper
         srvoDropper = opMode.hardwareMap.get(Servo.class, strDropperName);
         srvoDropper.setDirection(Servo.Direction.FORWARD);

         // Left pitstop
         srvoLeftPitstop = opMode.hardwareMap.get(Servo.class, strLeftPitstopName);
         srvoLeftPitstop.setDirection(Servo.Direction.REVERSE);
     ...
     // Autonomous Dropper
     public void autonMoveDropper(LinearOpMode opMode, String msg, boolean up, int timeout) {
         long lMarkMilliS;
         lMarkMilliS = System.currentTimeMillis() + timeout;
         if (up) {
             srvoDropper.setPosition(dDropperUpPos);
         } else {
             srvoDropper.setPosition(dDropperDownPos);
         }
         while ((System.currentTimeMillis() < lMarkMilliS) && opMode.opModeIsActive()) {
             opMode.telemetry.addData(msg + " Dropper pos:", "%.2f",
                     srvoDropper.getPosition());
             opMode.telemetry.update();
         }
     }
 ...
 */
     ...
     void telMoveShooter(Gamepad gamepad2) {
         ...
         if (gamepad2.right_trigger > (0.67)) {
             mtrShooter.setPower(dPwrShooterUp);
         } else if (gamepad2.right_bumper) {
             mtrShooter.setPower(dPwrShooterDown);
         } else {
